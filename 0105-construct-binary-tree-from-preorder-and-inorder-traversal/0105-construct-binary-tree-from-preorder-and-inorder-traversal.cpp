...
public:
    TreeNode *solve(vector<int> &preorder,int preStart,int preEnd,vector<int> &inorder,int inStart,int inEnd,unordered_map<int,int> &m)
    {
        ...
        int numbersOnLeft = rootInInorder - inStart;
        
        root->left = solve(preorder,preStart+1,preStart+numbersOnLeft,
                           inorder,inStart,rootInInorder-1,m);
        root->right = solve(preorder,preStart+numbersOnLeft+1,preEnd,
